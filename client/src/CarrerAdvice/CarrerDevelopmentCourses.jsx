...
export default function CarrerDevelopmentCourses() {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "http://localhost:3000/api/v1/career/category?category=Career Development"
        );
  ...
  useEffect(() => {
    const fetchDataa = async () => {
      try {
        const response = await axios.get(
          "http://localhost:3000/api/v1/career/imageandtitle?category=Career Development"
        );
    ...
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 p-8 items-center justify-center lg:grid-cols-3 gap-8">
        {dataa.slice(0, 3).map(
          (
            entry,
            index // Display only the first 3 entries
          ) => (
